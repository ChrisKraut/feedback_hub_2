...
func (h *RoleHandler) CreateRole(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		switch err {
		case role.ErrUnauthorized:
			writeErrorResponse(w, http.StatusForbidden, "Insufficient permissions")
		case role.ErrRoleNameAlreadyExists:
			writeErrorResponse(w, http.StatusConflict, "Role name already exists")
		default:
...
func (h *RoleHandler) GetRole(w http.ResponseWriter, r *http.Request) {
	...
	roleID := extractIDFromPath(r.URL.Path, "/roles/")
	if roleID == "" {
		writeErrorResponse(w, http.StatusBadRequest, "Role ID is required")
		return
	}
...
func (h *RoleHandler) UpdateRole(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case role.ErrRoleNameAlreadyExists:
			writeErrorResponse(w, http.StatusConflict, "Role name already exists")
		case role.ErrCannotModifySuperUserRole:
			writeErrorResponse(w, http.StatusBadRequest, "Cannot modify Super User role")
		default:
			writeErrorResponse(w, http.StatusInternalServerError, "Internal server error")
		}
		return
...
func (h *RoleHandler) DeleteRole(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case role.ErrRoleNotFound:
			writeErrorResponse(w, http.StatusNotFound, "Role not found")
		case role.ErrCannotDeleteSuperUserRole:
			writeErrorResponse(w, http.StatusBadRequest, "Cannot delete Super User role")
		case role.ErrInvalidRoleData:
			writeErrorResponse(w, http.StatusBadRequest, "Cannot delete role with assigned users")
		default:
			writeErrorResponse(w, http.StatusInternalServerError, "Internal server error")
		}
		return
